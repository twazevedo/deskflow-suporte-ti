...
import { Ticket, PRIORITY_MAP, CATEGORY_MAP } from '../types/ticket';
import { 
  UserCheck, 
  CheckCircle, 
  Clock, 
...
interface AgentDesksViewProps {
  ...
  onResolveTicket: (agentId: string, ticketId: string, resolutionNotes: string) => void;
...
export const AgentDesksView: React.FC<AgentDesksViewProps> = ({
  ...
  const handleResolveSubmit = (agentId: string, ticketId: string) => {
    const notes = resolutionNotesMap[agentId] || 'Chamado solucionado e validado junto ao usuário.';
    onResolveTicket(agentId, ticketId, notes);
                ...
                <div className="mt-4 space-y-3 pt-3 border-t border-slate-800/80">
                  ...
                  <div className="p-3.5 rounded-2xl bg-slate-950/80 border border-slate-800">
                    ...
                    <div className="mt-2.5 flex items-center justify-between text-[11px] text-slate-400 pt-2 border-t border-slate-800/80">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span>{currentTicket.requester.name}</span>
                        <span className="text-slate-500">({currentTicket.requester.department})</span>
                      </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <textarea
                      ...
                      placeholder="Ex: Realizado flush no DNS, túnel VPN restabelecido e teste validado junto ao usuário..."
                      className="w-full text-xs bg-slate-950 border border-slate-800 rounded-xl p-2.5 text-slate-100 placeholder:text-slate-600 focus:outline-none focus:border-indigo-500/60 focus:ring-1 focus:ring-indigo-500/50"
                    />
                  </div>

                  {/* Action Buttons: Resolve, Transfer */}
                  <div className="flex flex-col sm:flex-row items-center justify-between gap-2 pt-1">
                    ...
                    <div className="flex items-center gap-1.5 w-full sm:w-auto">
                      ...
                      >
                        <option value="">Transferir p/ analista...</option>
                        {agents
